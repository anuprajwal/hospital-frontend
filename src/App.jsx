...
export default function App() {
  const [activeTab, setActiveTab] = useState('staff'); // Default to staff tab to test directly
  const [selectedDoctor, setSelectedDoctor] = useState(null);
    ...
    <div class="min-h-screen flex flex-col bg-slate-50">
      ...
      <header class="sticky top-0 z-40 bg-slate-900 text-white shadow-md border-b border-slate-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
          ...
          <nav class="flex h-full items-center">
            <button 
              onClick={() => { setActiveTab('profile'); setSelectedDoctor(null); }} 
              class={`px-4 h-16 flex items-center gap-2 text-xs font-semibold border-b-2 transition-all ${activeTab === 'profile' ? 'border-blue-500 text-white bg-slate-800/50' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              <Building2 class="h-4 w-4" /> Company Profile
            </button>
            <button 
              onClick={() => { setActiveTab('staff'); setSelectedDoctor(null); }} 
              class={`px-4 h-16 flex items-center gap-2 text-xs font-semibold border-b-2 transition-all ${activeTab === 'staff' ? 'border-blue-500 text-white bg-slate-800/50' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              <Users2 class="h-4 w-4" /> Staff Registry
            </button>
            <button 
              onClick={() => { setActiveTab('appointments'); setSelectedDoctor(null); }} 
              class={`px-4 h-16 flex items-center gap-2 text-xs font-semibold border-b-2 transition-all ${activeTab === 'appointments' ? 'border-blue-500 text-white bg-slate-800/50' : 'border-transparent text-slate-400 hover:text-white'}`}
            >
              <CalendarDays class="h-4 w-4" /> Appointments
            </button>
          </nav>
      ...
      <main class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'staff' && (
          selectedDoctor ? (
            <DoctorDetailPage doctor={selectedDoctor} onBack={() => setSelectedDoctor(null)} />
          ) : (
